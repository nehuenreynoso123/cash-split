...
export async function listFlujoFondos({ desde, hasta } = {}) {
  ...
  const result = await sql`
    SELECT 
      ...
      COALESCE(SUM(CASE WHEN v.fecha_cobro IS NULL THEN v.precio::numeric - (p.precio::numeric * v.cantidad) ELSE 0 END), 0) AS ganancia_por_cobrar_total,
      COALESCE(SUM(CASE WHEN v.fecha_cobro IS NULL THEN v.cantidad ELSE 0 END), 0) AS unidades_por_cobrar
    FROM productos p
    ${joinFilter}
