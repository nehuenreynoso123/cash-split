...
import { createLiberacion } from '../../lib/api';
...
export default function LiberacionPlataModal({ open, onClose, onSaved }: Props) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setSaving(true);
    try {
      await createLiberacion({ fecha, hora, monto: Number(monto) });
      onSaved();
    } catch (err) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Nueva Liberación">
      <form onSubmit={handleSubmit} className="p-8 space-y-6">
        {error && <div className="bg-error-container text-on-error-container text-body-sm rounded-lg px-4 py-2">{error}</div>}
        <div>
          <label className="block font-label-caps text-label-caps text-on-surface-variant uppercase mb-2">Fecha</label>
          <input className="w-full px-4 py-2.5 border border-outline-variant rounded-lg focus:ring-2 focus:ring-secondary/20 focus:border-secondary outline-none" type="date" value={fecha} onChange={(e) => setFecha(e.target.value)} required />
        </div>
        <div>
          <label className="block font-label-caps text-label-caps text-on-surface-variant uppercase mb-2">Hora</label>
          <input className="w-full px-4 py-2.5 border border-outline-variant rounded-lg focus:ring-2 focus:ring-secondary/20 focus:border-secondary outline-none" type="time" value={hora} onChange={(e) => setHora(e.target.value)} required />
        </div>
        <div>
